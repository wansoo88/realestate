...
## 0. ⚠️ F4(동별 가치 차이) — 설계 가정 정정 (2026-07-25)

> **정정 이력.** 이 절의 초안(2026-07-24)은 "국토부 실거래가에는 `동(棟)` 정보가 없다"고
> 단정하고 F4를 좌표 추정으로만 설계했다. 이는 **개발용 엔드포인트(RTMSDataSvcAptTradeDev)만
> 보고 내린 판단이었고, 틀렸다.** 3단계에서 발급 키로 **운영 엔드포인트
> (RTMSDataSvcAptTrade)** 를 실호출한 결과, 응답에 `aptDong` 필드가 **77~93%** 채워져 온다
> (강남 87 · 분당 93 · 인천 91 · 종로 77%, 2026-07-25 실측). 아래는 정정된 설계다.

요구사항 **F4(같은 단지도 동·층·타입별 가치 차이)** 는 이 서비스의 차별점이다.
운영 API 가 `aptDong` 을 대부분 제공하므로, **동별 가치 차이를 실거래로 직접 측정**할 수 있다 —
다만 결측 10~23% 가 있어 그 부분은 좌표 추정으로 폴백한다.

공개되는 실거래 필드:
> 지역코드 · 단지명 · 전용면적 · **층** · **동(aptDong, 77~93%)** · 계약년월일 · 거래금액 · 건축년도 · 해제여부 · 거래유형

**대응 전략 (스키마에 반영됨)**

| 축 | 데이터 근거 | 가능 여부 |
|---|---|---|
| **층별** 편차 | 실거래 `floor` | ✅ 직접 계산 |
| **타입/면적별** 편차 | 실거래 `area_m2` → `unit_type` 매칭 | ✅ 직접 계산 |
| **동별** 편차 (aptDong 있음) | 실거래 `trade.apt_dong` 직접 집계 | ✅ **실측 (high confidence)** |
| **동별** 편차 (aptDong 결측 10~23%) | ① 호가 매물(`listing.building_id`) ② 동 좌표 기반 입지 추정 | ⚠️ 간접 추정 (폴백) |
| 향·조망 | 동 좌표 + 배치 + POI 거리 | ⚠️ 추정 |

**스키마 반영**
- `trade.apt_dong text` (마이그레이션 006) — 실거래 동 원본 표기 보존('410'·'청담(103)' 혼재).
  **자연키(004)에는 넣지 않는다** — 결측이 있어 키에 넣으면 같은 거래가 흩어진다
  (unit_type_id 를 키에서 뺀 것과 동일 논리). 부가 컬럼으로만 저장.
- `building` 테이블의 **좌표(`geom`)** 는 유지 — aptDong 이 없는 거래와 향·조망 추정에 계속 쓴다.

→ F4 는 `agent_finding.confidence` 로 **근거의 질을 구분**한다: aptDong 실측이면 high,
좌표 추정 폴백이면 낮춰 표기. `evidence` 컬럼에 어느 쪽인지 남긴다.

> **동별 판단이 실측인지 추정인지**를 UI·리포트에서 반드시 구분해 밝혀야 한다.
> 추정을 확정치처럼 보이게 만들면 G2(근거 감사) 위반이다.
...
erDiagram
    ...
    TRADE {
        ...
        smallint floor "층"
        numeric area_m2
        text apt_dong "동(棟) — 운영API 77~93% 제공, 자연키 아님(006)"
...
- F4 동별 실측용: `(complex_id, apt_dong)` where `apt_dong IS NOT NULL` (부분 인덱스, 006 후속)
