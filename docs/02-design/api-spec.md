...
> ⚠️ **이 항목은 오래 지도에만 적용됐다** (CR39-2, 2026-07-30에 해소)
> 위 문단이 "예전에는"으로 시작하는 탓에 **추천도 고쳐진 것처럼 읽혔지만**, 추천 러너는
> 2026-07-30까지 기본값 84.0 으로 만든 한 숫자로 후보를 하드 제외하고 있었다.
> 지금은 지도·추천이 **같은 함수**(`app/domain/affordability/budget.py`)를 쓴다 —
> 추천 쪽 계약은 [§5 희망 매매가](#희망-매매가--budget_override_krw-2026-07-26) 뒤의
> **📐 후보 예산 상한** 항목에 있다. 한쪽만 고치면 같은 단지가 두 화면에서 다른 답을
> 받고, 그 어긋남을 아무 테스트도 보지 않는다.
...
#### 📐 후보 예산 상한도 **하나의 숫자가 아니다** (CR39-2, 2026-07-30)

후보의 판정 단위는 **단지 × 면적대**다(위 문단). 그리고 자산으로 계산한 상한은
**전용면적의 함수**다(§4 의 표 — 취득세 농특세 85㎡ 경계 · 차이 1,980,000원 · 0.19%).
그러므로 한 숫자로 후보 전체를 판정할 수 없다. 지금 계약은 이렇다:

```
제외 = est_price_krw > max_purchase(area = unit_type.area_m2)
```

* **후보마다 그 후보 면적으로 상한을 세운다.** 따라서 같은 면적으로
  `POST /affordability` 를 부르면 **같은 `max_purchase_krw`** 가 나오고,
  제외 사유 문장에 적히는 한도도 그 숫자다(사용자가 되짚을 수 있어야 한다).
  `backend/tests/test_recommend_budget_parity.py` 가 API 전 구간에서 못박는다
  (픽스처 세율로는 재현되지 않으므로 그 파일만 운영 세율을 쓴다 — 지도 쪽과 같은 이유).
* ⚠️ **이건 하드 제외다** — 지도의 배지와 달리 후보가 목록에서 사라진다. 그래서
  ① 제외에는 항상 `reason_code`+문장이 남고 ② `items + excluded` 가 후보 총수와 같다
  (어디에도 없는 후보 = 조용한 유실).
* **전용면적이 확인되지 않은 후보**(수집 결손)는 상한을 세울 수 없다. 제외하지도,
  84 를 가정하지도 않는다 — **판정하지 않고 남기고 `notes` 로 건수를 말한다.**
  제외하면 "판정 못 함"이 "못 산다"가 되고, 84 를 가정하면 *다른 면적의 한도*로 내린
  판정이 된다. `budget_gap_krw` 도 그 후보에서는 `null` 이다.
* `budget_override_krw`(희망 매매가)를 준 경우에는 **면적과 무관한 금액 하나**다.
  그때는 면적 미상 후보도 판정한다(사용자가 정한 금액이므로 세율 구간이 필요 없다).
* 계산은 **세율 구간별로 1회**만 돈다(`acquisition_area_class` — 운영 세율이면 2회).
* ⚠️ 리포지토리(`recommendation_candidates`)에 넘기는 `max_price_krw` 는 **정렬 신호**다
  (거르지 않는다). 그래서 면적별 상한과 어긋나도 후보가 조회 단계에서 사라지지 않는다.

**결과 전체에 붙는 요약 문구는 기준 면적 하나로 만들고, 그 사실을 문장에 적는다** —
*"…최대 실구매 가능 금액(전용 84㎡ 기준) 1,026,560,000원을 …"*. 한 문장에 후보마다 다른
한도를 담을 수 없기 때문이고, 문장이 판정보다 강한 주장을 하지 않게 하려는 것이다.
기준 면적은 `AffordabilityIn.area_m2` 기본값과 **같은 값**이다(다르면 "내 정보" 화면의
한도와 이 고지의 한도가 서로 다른 숫자가 된다).

각 추천 카드에는 **적용 예산 대비 차액**이 실린다(프론트가 "희망가보다 1.2억 저렴"을 표시).

| 필드 | 뜻 |
|---|---|
| `budget_gap_krw` | `est_price_krw − 적용예산(그 후보 면적의 상한)`. **음수 = 예산보다 싸다**. 예산을 모르거나 상한을 세울 수 없으면 `null` |
