...
### C. 매물 신뢰도 (`trust_score`)
**부호 규약: 1.0 = 신뢰할 만함, 0.0 = 매우 의심스러움.** (의심도가 아니다 — 반대로 쓰면 정렬이 뒤집힌다)

| 신호 | 신뢰도 |
|---|---|
| 동일 단지·타입 중위 실거래 대비 **-15% 이하 호가** | ↓ (미끼 가능) |
| **-25% 이하** | ↓↓ |
| 등록 후 90일 이상 활성 | ↓ (실재하지 않거나 안 팔리는 물건) |
| 중복 등록 중개사 수 과다(4개↑, 8개↑) | ↓ |
| 최근(24시간 내) 수집으로 살아있음 확인 | ↑ 소폭 |

> **시세를 모르면 가격으로 감점하지 않는다.** 모르는 걸 근거로 깎는 것도 환각이다.
> 구현: `app/domain/listings/dedup.py::trust_score`
