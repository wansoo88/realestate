...
### 2.1 [4] 종합 — 사용자 가중치가 순위를 정한다 (2026-07-26)

`portfolio-advisor`의 "가중 종합"은 **LLM이 하는 일이 아니다.** 순위는 규칙 계산으로
정하고(`app/agents/scoring.py`), LLM은 정해진 순위에 설명을 붙인다(원칙 1).

| 축(`user_preference.weights`) | 담당 | 점수로 쓰는 신호 | 커버리지 |
|---|---|---|---|
| `price` 가격 | `valuation-trader` | 호가−적정가 밴드 갭 | full |
| `location` 입지 | `location-analyst` | 학군·역세권·인프라 근접 | full |
| `value` 가치(시세) | `valuation-trader` | 12개월 거래회전율(환금성) | partial (동별 편차는 점수화 안 함) |
| `risk` 리스크 | `listing-researcher` | 매물 신뢰도(허위·중복) | partial (**`risk-auditor` 2차 → 권리관계·깡통전세 미반영**) |

- `finance-tax-advisor`는 축이 아니다 — 예산은 취향이 아니라 **하드 제외**다.
- **근거 없는 축은 총점에서 빼고 재정규화**한다. 0점 처리(=나쁨)도, 조용한 무시도 하지 않는다.
- 빠진 축은 **비율·사유**와 함께 응답에 남긴다. `partial` 축은 반영 여부와 무관하게
  "어디까지만 보는지"를 말한다 — 안 그러면 미구현이 데이터 부족으로 위장된다.
- 가중치가 없으면 **기존 동작**(신뢰도 가중 평균)으로 폴백하고 그 사실을 남긴다.
- 가중치 축에 근거가 하나도 없으면 `total_score=null`(모름). 상세는 `api-spec.md` §5.3.
