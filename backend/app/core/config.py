...
class Settings(BaseSettings):
    ...
    def _runtime_checks(self) -> list[tuple[bool, str]]:
        ...
        *값이 잘못돼도 앱이 정상처럼 계속 도는가.* 그런 항목만 막는다(SR29-1).
        ...
        #
        #   ⚠️ 여기는 **문자 수**를 재고 메시지도 "자"라고 말한다 — 일부러 그렇다.
        #      UTF-8 에서 문자 수 ≤ 바이트 수이므로 32자를 요구하면 32바이트는 자동으로
        #      만족된다(RFC 하한을 넘긴다). 바이트로 바꾸면 오히려 **느슨해진다**
        #      (`'가'*11` = 11자 / 33바이트가 통과한다). FEK 쪽과 방향이 반대인 이유는
        #      FEK 가 "정확히 32바이트"라는 알고리즘 요구인 반면 이건 하한이기 때문이다.
        ...
        #
        # ⚠️ **바이트를 센다. 문자를 세면 안 된다**(SR30-1).
        #   예전에는 `len(str)` 로 문자 수를 재면서 메시지는 "32바이트"라고 말했다.
        #   비ASCII 키에서 둘은 다르다 — 실측: `'가'*32` = 32자 / **96바이트**,
        #   `'x'*31+'가'` = 32자 / **34바이트**. 둘 다 이 게이트를 통과한 뒤
        #   `security.load_key`(바이트를 잰다)에서 걸려 **첫 사용에서 503** 이 됐다.
        #   그러면 이 조치가 내건 목적("자산 저장 순간 500 이 되느니 기동 때 알자")이
        #   정확히 그 경우에만 달성되지 않는다. 재는 것과 말하는 것을 맞춘다.
        #   (현 운영 키는 순수 ASCII 라 실해는 없었다 — 그래도 방어가 방어인 척하면 안 된다.)
        fek_bytes = len(self.field_encryption_key.encode())
        if fek_bytes != 32:
            problems.append((
                True,
                f"FIELD_ENCRYPTION_KEY 는 정확히 32바이트여야 합니다 (현재 {fek_bytes}바이트)"))
