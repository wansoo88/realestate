...
AXIS_SPECS: dict[str, AxisSpec] = {
    AXIS_PRICE: AxisSpec(
        ...
        #    **대부분의 후보에 대해 점수가 나오지 않는다**(2026-07-28 실측: listing 0행).
        #    "설계한 신호를 다 본다"고 적어 두면 값이 하나도 없는 이유가 후보별
        #    `missing` 에만 묻히고, 축 설명만 읽는 사람은 가격을 따졌다고 믿는다.
        # ⚠️ 그렇다고 "**모든** 후보에서 점수가 나오지 않는다"고 적어 두면, 사용자가
        #    호가를 직접 입력한 순간(migrations/016) 그 문장이 거짓이 된다 — 그 후보에는
        #    실제로 가격 점수가 나온다(실측 2026-07-29: 가격 축 no_signal → applied
        #    53.35점 · coverage 20.0% → 68.0%). 고정 문장으로 사실을 덮지 않는다.
        coverage=COVERAGE_PARTIAL,
        coverage_gap=(
            "이 축은 **호가(매도 희망가)와 적정가의 차이**를 재는데, 호가가 확인된 "
            "후보에서만 점수가 나옵니다. 공공 오픈API 에는 호가가 없고 포털 매물 수집은 "
            "약관·판례상 하지 않아, 호가는 **직접 입력하신 것만** 있습니다"
            "(내 매물 → 호가 입력). 호가가 없는 후보에는 실거래 기반 적정가 밴드를 "
            "'추정가'로 제시하지만, 그건 '이 매물이 싼가'를 판정한 값이 아니라 "
            "'이 단지가 얼마쯤인가'라서 후보 간 순위로 쓰지 않습니다."),
    ),
...
# --- 신호가 없을 때의 표준 사유 ---------------------------------------------
#: ⚠️ **막다른 길로 끝내지 않는다.** 예전 문구는 "공공 오픈API 에는 호가가 없습니다"
#:    까지만 말했다 — 사실이지만, 그걸 읽은 사용자가 할 수 있는 일이 없다.
#:    호가를 얻는 길이 실제로 하나 남아 있고(직접 입력 · migrations/016) 그 길을
#:    타면 이 축이 살아난다(실측: 가격 축 no_signal → applied). 그 사실을 말한다.
NO_ASK_REASON = ("활성 호가가 없어 호가–적정가 갭을 계산할 수 없습니다 "
                 "(공공 오픈API 에는 호가가 포함되지 않고, 포털 매물 수집은 "
                 "약관·판례상 하지 않습니다). 이 단지의 호가를 알고 계시면 "
...
def summary_notes(*, weights: dict[str, float], unknown_keys: list[str],
    ...
    for axis in WEIGHT_AXES:
        ...
        # ⚠️ 사유 문자열이 이미 문장부호로 끝날 수 있다(`NO_ASK_REASON` 등).
        #    그대로 `. ` 를 덧붙이면 화면에 "…반영됩니다.. 그만큼" 으로 나간다 —
        #    사용자가 보는 문장이므로 여기서 한 번 다듬는다(CR35-2 덤).
        detail = "; ".join(sorted(reasons[axis])).rstrip(" .")
        notes.append(
            f"{AXIS_SPECS[axis].label} 가중치 {_pct(weights.get(axis, 0.0))}가 "
            f"{scope}에서 반영되지 않았습니다 — {detail}. "
