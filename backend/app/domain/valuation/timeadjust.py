...
3. **기준월은 '오늘'이 아니다.** 신고기한(계약일+30일)과 그 뒤의 공개 지연 때문에
   이번 달은 표본이 덜 차 있다(30일은 기한이지 지연의 상한이 아니다 — `REPORT_LAG_DAYS`).
...
#: 신고기한(일). 부동산 거래신고 등에 관한 법률 제3조 제1항 — 계약 **체결일**로부터
#: **30일 이내** 신고(2020-02-21 시행. 그 전은 60일이나 해당 월은 6년 전이라 무영향).
#: 그래서 어떤 달의 거래가 다 신고됐는지는 **그 달 말일 + 30일**이 지나야 말할 수 있다.
#:
#: ⚠️ **이 30일은 '신고기한'이지 'API 공개 지연'이 아니다**(CR34-4). 이 상수가 덮는 것과
#:    덮지 못하는 것을 분명히 한다:
#:      덮는 것   — 기한 안에 낸 신고가 아직 안 들어온 구간
#:      못 덮는 것 — ① 신고분이 공개 API 에 실리기까지 걸리는 추가 시간
#:                   ② 지각 신고(기한을 넘겨 낸 것)  ③ 해제신고(뒤늦게 취소로 바뀌는 것)
#:    즉 이 경계는 **"완결이라고 말할 수 있는 가장 이른 날"** 이지 보수적인 값이 아니다.
#:    잔여 흔들림은 지각 신고율만큼이고 건수 검사(`MIN_MONTH_COMPLETENESS`)가 큰 누락은
#:    잡는다. 며칠 여유(33~35일)를 더하는 것은 값싼 강화지만 **근거 없는 숫자를 넣지
#:    않는다** — 넣으려면 지각 신고율·공개 지연을 먼저 재고 그 실측값을 여기 적을 것.
#:    (우리는 아직 재지 않았다. 재지 않은 값을 상수로 박는 것이 이 결함의 원형이다.)
REPORT_LAG_DAYS = 30
...
class IndexPoint:
    ...
    #:
    #: ⚠️ **그러나 환산 원천으로는 쓴다**(CR34-2). `ratio_to_reference` 는 이 값을
    #:    보지 않으므로 미완결 월의 거래도 그 달 지수로 환산된다. 그 지수에는
    #:    월중 편향이 들어 있다 — 월중에 신고된 거래는 무작위 표본이 아니고,
    #:    실측상 같은 (단지,면적,계약월) 안에서 미등기가 등기보다 +0.5~2.2% 비싸다.
    #:    방향은 **과소보정**이고 12개월 창에서 가중치가 2/12 라 중위 영향은 0.5% 미만이다.
    #:    미완결 월을 원천에서 빼면 6개월 창의 커버리지가 67% < 80% 가 되어 짧은 창의
    #:    보정이 통째로 꺼진다 — 그래서 **일부러 쓴다.** 모르고 쓰는 것과 다르다.
